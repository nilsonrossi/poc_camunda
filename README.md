...
- 🎯 Entry points to execute your program using `python -m <poc_camunda>` or `$ poc_camunda` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/nilsonrossi/poc_camunda/branch/main/graph/badge.svg?token=poc_camunda_token_here)](https://codecov.io/gh/nilsonrossi/poc_camunda)
[![CI](https://github.com/nilsonrossi/poc_camunda/actions/workflows/main.yml/badge.svg)](https://github.com/nilsonrossi/poc_camunda/actions/workflows/main.yml)

Awesome poc_camunda created by nilsonrossi

## Install

from source
```bash
git clone https://github.com/nilsonrossi/poc_camunda poc_camunda
cd poc_camunda
make install
```
...
```bash
pip install poc_camunda
```

## Executing

```bash
$ poc_camunda run --port 8080
```

or

```bash
python -m poc_camunda run --port 8080
```

or

```bash
$ uvicorn poc_camunda:app
```
...
```bash
❯ poc_camunda --help
Usage: poc_camunda [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ poc_camunda create-user --help
Usage: poc_camunda create-user [OPTIONS] USERNAME PASSWORD
...
```bash
poc_camunda create-user admin admin --superuser
```
...
Run with `poc_camunda run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `poc_camunda create-user`.
...
-----------------------------------------------------
poc_camunda/__init__.py              4      0   100%
poc_camunda/app.py                  16      1    94%
poc_camunda/cli.py                  21      0   100%
poc_camunda/config.py                5      0   100%
poc_camunda/db.py                   10      0   100%
poc_camunda/models/__init__.py       0      0   100%
poc_camunda/models/content.py       47      1    98%
poc_camunda/routes/__init__.py      11      0   100%
poc_camunda/routes/content.py       52     25    52%
poc_camunda/routes/security.py      15      1    93%
poc_camunda/routes/user.py          52     26    50%
poc_camunda/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from poc_camunda.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in poc_camunda/default.toml.

### As environment variables
```bash
export poc_camunda_KEY=value
export poc_camunda_KEY="@int 42"
export poc_camunda_KEY="@jinja {{ this.db.uri }}"
export poc_camunda_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
poc_camunda_ENV=production poc_camunda run
```
